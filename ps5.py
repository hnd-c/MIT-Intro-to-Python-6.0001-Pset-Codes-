...
def img_to_pix(filename):
    ...
    pix = im.getdata() #getting the data from image
    return list(pix)  #returning the list
...
def pix_to_img(pixels, size, mode):
    ...
    new.putdata(pixels,1.0,0.0) #filling the new black image with data
    return new
...
def filter(pixels, color):
    ...
    for tup in pixels:
        temp_list=[]
        lis=matrix_multiply(make_matrix(color),tup) #multiply each tuple with matirix_color
        for elm in lis:               
            temp_list.append(int(elm))         #converting every element of list into int   
        pixel_list.append(tuple(temp_list))    #add the tuples in the list
...
def extract_end_bits(num_bits, pixel):
    ...
    if type(pixel)==int:
        return pixel%2**num_bits #if int return the LSV
    else:
        tup_list=[]
        for num in pixel:
            ans=num%2**num_bits  #for every element in tuple return lsv
            tup_list.append(ans)  #add back the elements to a list
        return tuple(tup_list)   #return the tuple version of list
...
def reveal_bw_image(filename):
    ...
    ls_pixels=[]
    for elem in pixels:
        ans=extract_end_bits(1,elem)  #for every element in pixel return lsv
        ls_pixels.append(ans*255)     #scale the elements by 255:bw
    
    
    im=Image.open(filename)
    siz=im.size
    mode='L'
    return pix_to_img(ls_pixels,siz,mode) #convert the data into an image
...
def reveal_color_image(filename):
    ...
    ls_pixels=[]
    for elem in pixels:
        ans=extract_end_bits(2,elem) #for every tuple in a list
        tem_list=[]
        for elm in ans:
            ans2=elm*85      #for every element in a tuple, rescale the value by 255/3
            tem_list.append(ans2)
        ls_pixels.append(tuple(tem_list)) #assembe the elements in tuple and tuples in a list
